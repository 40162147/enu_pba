...
void DrawCube(const glm::mat4 &m, const RGBAInt32 col) {
  static geometry geom = geometry_builder::create_box(glm::vec3());
  renderer::bind(effP);
  auto M = m;
  mat3 N(1.0f);
  mat.set_diffuse(col.tovec4());
...
void DrawCylinder(const glm::vec3 &p0, float dia, float height, const RGBAInt32 col) {
	static geometry geom = geometry_builder::create_cylinder(20, 20);
	renderer::bind(effP);
	auto M = glm::translate(mat4(1.0f), p0) * glm::scale(mat4(height), vec3(dia));
	mat3 N(1.0f);
	mat.set_diffuse(col.tovec4());
	renderer::bind(mat, "mat");
	renderer::bind(light, "light");
...
void DrawCylinder(const glm::mat4 &m, float dia, float height, const RGBAInt32 col) {
	static geometry geom = geometry_builder::create_cylinder(20, 20, glm::vec3(dia, height, dia));
	renderer::bind(effP);
	auto M = m;
	mat3 N(1.0f);
	mat.set_diffuse(col.tovec4());
