...
const float Entity::GetDiam() const { return diam_; }
const float Entity::Getheightm() const { return heightm_; }
...
void Entity::SetDiam(float const &diam) { diam_ = diam; }
void Entity::SetHeightm(float const &heightm) { heightm_ = heightm; }
...
void cShapeRenderer::Render() {
  ...
  float dia = Ent_->GetDiam();
  float height = Ent_->Getheightm();
  ...
  case BOX:
    ...
    break;
  case CYLINDER:
	  phys::DrawCylinder(m, dia, height,  col_);
	  break;
  default:
