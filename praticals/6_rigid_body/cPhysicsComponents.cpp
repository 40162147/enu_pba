...
void cRigidCylinder::ComputeLocalInvInertiaTensor()
{
	...
	const double Iz = (mass *((dia/2) * (dia/2)))/2 ;
	const double Ixy = (mass / 12)*(3 * ((dia / 2) * (dia / 2)) + (height * height));

	
	dmat3 localInvInertia = dmat3(0.0);
	localInvInertia[0][0] = Ixy;
	localInvInertia[1][1] = Ixy;
	localInvInertia[2][2] = Iz;
	worldInvInertia = mat4_cast(orientation) * dmat4(localInvInertia) * transpose(mat4_cast(orientation));
}
